...
static void do_dram_init(struct dram_para * para)
{
  ...
  val = readl(SUNXI_PIO_BASE + (4 * 0x24) + 0x00) & 0xfffff0ff;
  writel(val, SUNXI_PIO_BASE + ((4 * 0x24) + 0x00));
  writel(0x00000010, SUNXI_PIO_BASE + ((4 * 0x24) + 0x1c));
  udelay(5000);
  m = readl(SUNXI_PIO_BASE + (4 * 0x24) + 0x10) & 4;
  if(m == 0){
    printf("(use default 156MHz)");
    para->clk = 156;
  }

  if((para->clk < 156) || (para->clk > 252)){
    printf("(adjust dram clock from 0x%x to 156MHz)", para->clk);
    para->clk = 156;
  }
